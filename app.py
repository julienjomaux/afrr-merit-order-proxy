...
def build_dashboard_payload(df: pd.DataFrame) -> dict:
    ...
    daily = {}
    grp_avg = df.groupby(["DAY", "DIRECTION", "BLOCK"])[AVERAGE_COL].mean()
    grp_marg = df.groupby(["DAY", "DIRECTION", "BLOCK"])[MARGINAL_COL].mean()

    for day in sorted(df["DAY"].unique()):
        def get_vals(grp, direction):
            vals = []
            for block in BLOCKS:
                key = (day, direction, block)
                if key in grp.index:
                    vals.append(round(float(grp.loc[key]), 2))
                else:
                    vals.append(None)
            return vals

        daily[day] = {
            "up_avg": get_vals(grp_avg, "POS"),
            "down_avg": get_vals(grp_avg, "NEG"),
            "up_marg": get_vals(grp_marg, "POS"),
            "down_marg": get_vals(grp_marg, "NEG"),
        }
